...
export default function DatePicker({
  ...
  return (
    <div ref={containerRef} className={`relative flex flex-col ${className}`}>
      {label && (
        <span className="block text-[10px] font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider mb-1 text-left">
          {label}
        </span>
      )}
      <button
        ...
        className="flex items-center justify-between w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100/70 dark:hover:bg-slate-700/80 border border-slate-200 dark:border-slate-800 rounded-xl text-left text-sm font-semibold text-slate-700 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-[#0091EA] transition-all"
      >
      ...
      {isOpen && (
        <div className="absolute left-0 mt-2 z-50 w-72 bg-white dark:bg-slate-950 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-800 p-4 animate-fade-in text-left" style={{ top: '100%' }}>
          {/* Header */}
