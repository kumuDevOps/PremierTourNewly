...
export default function BookingPDFModal({ isOpen, onClose, booking }: BookingPDFModalProps) {
  ...
  const translateDescription = (desc?: string) => {
    if (!desc) return '';
    let result = translate(desc);
    result = result.replace(/Confirmed Booking Voucher for/gi, translate('Confirmed Booking Voucher for'));
    result = result.replace(/Add-ons:/gi, translate('Add-ons:'));
    result = result.replace(/Hotel:/gi, translate('Hotel:'));
    result = result.replace(/Vehicle:/gi, translate('Vehicle:'));
    result = result.replace(/With Driver/gi, translate('With Driver'));
    result = result.replace(/Self-Drive/gi, translate('Self-Drive'));
    result = result.replace(/Valued Guest|Guest/gi, (m) => translate(m));
    result = result.replace(/Private Luxury Chauffeur Rental Pass for/gi, translate('Private Luxury Chauffeur Rental Pass for'));
    result = result.replace(/Full insurance, fuel, and highway tolls included\./gi, translate('Full insurance, fuel, and highway tolls included.'));
    result = result.replace(/Pick-up at/gi, translate('Pick-up at'));
    result = result.replace(/Pick-down at|Drop-off at/gi, translate('Drop-Off Location:'));
    result = result.replace(/Luxury Stay Booking Voucher for/gi, translate('Luxury Stay Booking Voucher for'));
    result = result.replace(/Confirmed reservation at/gi, translate('Confirmed reservation at'));
    result = result.replace(/1 nights/gi, `1 ${translate('nights')}`);
    result = result.replace(/1 days/gi, `1 ${translate('days')}`);
    return result;
  };
  ...
  const getServiceLabel = () => {
    switch (booking?.type?.toLowerCase()) {
      case 'hotel':
        return translate('Luxury Hotel & Suite Reservation');
      case 'car':
        return translate('Private Vehicle Rental Pass');
      case 'flight':
        return translate('Flight Travel Boarding Voucher');
      default:
        return translate('Tour & Holiday Package Voucher');
    }
  };
  ...
  const resolvedRoomType = booking?.roomType || translate('Executive Deluxe Ocean View');
  const resolvedDuration = booking?.durationText || (booking?.type === 'hotel' ? `3 ${translate('nights')} / 4 ${translate('days')}` : translate('Full Day Experience'));
  const resolvedStartTime = booking?.startTime || '09:00 AM (EST)';
  const destinationLoc = booking?.hotelLocation || booking?.flightTo || booking?.subtitle || booking?.title || translate('Premier Luxury Destination');
  ...
  const buildLuxuryVectorPDF = (bookingData: BookingVoucherData) => {
    ...
    const serviceLabel = bookingData.type === 'hotel' 
      ? translate('LUXURY HOTEL & SUITE RESERVATION') 
      : bookingData.type === 'car' 
      ? translate('PRIVATE VEHICLE RENTAL PASS') 
      : bookingData.type === 'flight' 
      ? translate('FLIGHT TRAVEL BOARDING VOUCHER') 
      : translate('TOUR & HOLIDAY PACKAGE VOUCHER');
    ...
    pdf.setFontSize(11);
    pdf.text(translate('PTB'), margin + 11, 23, { align: 'center' });

    // Brand Name & Subtitle
    pdf.setTextColor(255, 255, 255);
    pdf.setFontSize(15);
    pdf.setFont('helvetica', 'bold');
    pdf.text(translate('Premier Tour Booking'), margin + 22, 20);

    pdf.setTextColor(56, 189, 248); // #38BDF8
    pdf.setFontSize(7);
    pdf.setFont('helvetica', 'bold');
    pdf.text(translate('GLOBAL LUXURY TRAVEL & EXPERIENCES LTD.'), margin + 22, 25);
    ...
    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(7);
    pdf.text(translate('OFFICIAL VOUCHER & RECEIPT'), pageWidth - margin - 32, 19.5, { align: 'center' });
    ...
    pdf.text(`${translate('Issued')}: ${issueDateStr}`, pageWidth - margin - 4, 32, { align: 'right' });
    ...
    pdf.setFont('helvetica', 'bold');
    const titleText = translate(bookingData.title);
    const displayTitle = titleText.length > 50 ? titleText.substring(0, 47) + '...' : titleText;
    pdf.text(displayTitle, margin + 6, heroY + 16);

    // Subtitle / Category
    const rawSubtitle = bookingData.subtitle || bookingData.category || 'Luxury Travel Experience';
    const subtitleText = translate(rawSubtitle);
    pdf.setTextColor(148, 163, 184);
    pdf.setFontSize(8);
    pdf.setFont('helvetica', 'normal');
    ...
    pdf.setFontSize(6.5);
    pdf.setFont('helvetica', 'bold');
    pdf.text(translate('STATUS'), margin + 6, heroY + 28);
    pdf.setTextColor(74, 222, 128); // #4ADE80
    pdf.setFontSize(8.5);
    pdf.text(`[OK] ${translate(bookingData.status || 'CONFIRMED').toUpperCase()}`, margin + 6, heroY + 33);

    // Col 2: Start Date
    pdf.setTextColor(148, 163, 184);
    pdf.setFontSize(6.5);
    pdf.setFont('helvetica', 'bold');
    pdf.text(translate('START DATE'), margin + 6 + colW, heroY + 28);
    pdf.setTextColor(255, 255, 255);
    pdf.setFontSize(8.5);
    pdf.text(translate(bookingData.startDate || 'Aug 15, 2026'), margin + 6 + colW, heroY + 33);

    // Col 3: Start Time
    pdf.setTextColor(148, 163, 184);
    pdf.setFontSize(6.5);
    pdf.setFont('helvetica', 'bold');
    pdf.text(translate('START TIME'), margin + 6 + colW * 2, heroY + 28);
    pdf.setTextColor(255, 255, 255);
    pdf.setFontSize(8.5);
    pdf.text(translate(resolvedStartTime), margin + 6 + colW * 2, heroY + 33);

    // Col 4: Duration
    pdf.setTextColor(148, 163, 184);
    pdf.setFontSize(6.5);
    pdf.setFont('helvetica', 'bold');
    pdf.text(translate('DURATION'), margin + 6 + colW * 3, heroY + 28);
    pdf.setTextColor(56, 189, 248);
    pdf.setFontSize(8.5);
    pdf.text(translate(resolvedDuration), margin + 6 + colW * 3, heroY + 33);
    ...
    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(7.5);
    pdf.text(translate('LEAD GUEST DETAILS & CONTACT'), margin + 5, cardY + 6);
    ...
    pdf.setFontSize(7.5);
    pdf.setFont('helvetica', 'normal');
    pdf.text(`${translate('Email')}: ${bookingData.customerEmail}`, margin + 5, cardY + 22);
    pdf.text(`${translate('Phone')}: ${bookingData.customerPhone || '+1 800-555-0199'}`, margin + 5, cardY + 27);
    pdf.text(`${translate('Party Size')}: ${bookingData.guestsCount || 1} ${translate('Person(s)')}`, margin + 5, cardY + 32);

    pdf.setTextColor(5, 150, 105);
    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(7);
    pdf.text(`[OK] ${translate('Verified Lead Guest & Online Payment')}`, margin + 5, cardY + 40);
    ...
    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(7.5);
    pdf.text(translate('STAY & FACILITY SPECIFICATIONS'), rightCardX + 5, cardY + 6);

    pdf.setFontSize(7.5);
    if (bookingData.type === 'hotel') {
      pdf.setTextColor(100, 116, 139); pdf.setFont('helvetica', 'normal'); pdf.text(translate('Assigned Room:'), rightCardX + 5, cardY + 16);
      pdf.setTextColor(0, 145, 234); pdf.setFont('helvetica', 'bold'); pdf.text(translate(resolvedRoomNumber), rightCardX + 42, cardY + 16);

      pdf.setTextColor(100, 116, 139); pdf.setFont('helvetica', 'normal'); pdf.text(translate('Room Category:'), rightCardX + 5, cardY + 22);
      pdf.setTextColor(15, 23, 42); pdf.setFont('helvetica', 'bold'); pdf.text(translate(resolvedRoomType).substring(0, 22), rightCardX + 42, cardY + 22);

      pdf.setTextColor(100, 116, 139); pdf.setFont('helvetica', 'normal'); pdf.text(translate('Check-Out Date:'), rightCardX + 5, cardY + 27);
      pdf.setTextColor(15, 23, 42); pdf.setFont('helvetica', 'bold'); pdf.text(translate(bookingData.endDate || 'Standard Checkout'), rightCardX + 42, cardY + 27);

      pdf.setTextColor(100, 116, 139); pdf.setFont('helvetica', 'normal'); pdf.text(translate('Inclusions:'), rightCardX + 5, cardY + 32);
      pdf.setTextColor(5, 150, 105); pdf.setFont('helvetica', 'bold'); pdf.text(translate('Free Breakfast & Wi-Fi'), rightCardX + 42, cardY + 32);
    } else if (bookingData.type === 'car') {
      pdf.setTextColor(100, 116, 139); pdf.setFont('helvetica', 'normal'); pdf.text(translate('Category:'), rightCardX + 5, cardY + 16);
      pdf.setTextColor(15, 23, 42); pdf.setFont('helvetica', 'bold'); pdf.text(translate(bookingData.carCategory || 'Luxury Vehicle'), rightCardX + 42, cardY + 16);

      pdf.setTextColor(100, 116, 139); pdf.setFont('helvetica', 'normal'); pdf.text(translate('Return Date:'), rightCardX + 5, cardY + 22);
      pdf.setTextColor(15, 23, 42); pdf.setFont('helvetica', 'bold'); pdf.text(translate(bookingData.endDate || bookingData.startDate), rightCardX + 42, cardY + 22);

      pdf.setTextColor(100, 116, 139); pdf.setFont('helvetica', 'normal'); pdf.text(translate('Transmission:'), rightCardX + 5, cardY + 27);
      pdf.setTextColor(15, 23, 42); pdf.setFont('helvetica', 'bold'); pdf.text(translate(bookingData.carTransmission || 'Automatic'), rightCardX + 42, cardY + 27);

      pdf.setTextColor(100, 116, 139); pdf.setFont('helvetica', 'normal'); pdf.text(translate('Mileage:'), rightCardX + 5, cardY + 32);
      pdf.setTextColor(5, 150, 105); pdf.setFont('helvetica', 'bold'); pdf.text(translate('Unlimited Included'), rightCardX + 42, cardY + 32);
    } else if (bookingData.type === 'flight') {
      pdf.setTextColor(100, 116, 139); pdf.setFont('helvetica', 'normal'); pdf.text(translate('Airline:'), rightCardX + 5, cardY + 16);
      pdf.setTextColor(15, 23, 42); pdf.setFont('helvetica', 'bold'); pdf.text(translate(bookingData.flightAirline || 'Emirates Air'), rightCardX + 42, cardY + 16);

      pdf.setTextColor(100, 116, 139); pdf.setFont('helvetica', 'normal'); pdf.text(translate('Route:'), rightCardX + 5, cardY + 22);
      ...
      pdf.setTextColor(100, 116, 139); pdf.setFont('helvetica', 'normal'); pdf.text(translate('Cabin Class:'), rightCardX + 5, cardY + 27);
      pdf.setTextColor(15, 23, 42); pdf.setFont('helvetica', 'bold'); pdf.text(translate('Business Class'), rightCardX + 42, cardY + 27);

      pdf.setTextColor(100, 116, 139); pdf.setFont('helvetica', 'normal'); pdf.text(translate('Baggage:'), rightCardX + 5, cardY + 32);
      pdf.setTextColor(5, 150, 105); pdf.setFont('helvetica', 'bold'); pdf.text(translate('32kg Checked Bag'), rightCardX + 42, cardY + 32);
    } else {
      pdf.setTextColor(100, 116, 139); pdf.setFont('helvetica', 'normal'); pdf.text(translate('Package Type:'), rightCardX + 5, cardY + 16);
      pdf.setTextColor(0, 145, 234); pdf.setFont('helvetica', 'bold'); pdf.text(translate(bookingData.category || 'All-Inclusive Tour'), rightCardX + 42, cardY + 16);

      pdf.setTextColor(100, 116, 139); pdf.setFont('helvetica', 'normal'); pdf.text(translate('Scheduled Guide:'), rightCardX + 5, cardY + 22);
      pdf.setTextColor(15, 23, 42); pdf.setFont('helvetica', 'bold'); pdf.text(translate('Licensed English Guide'), rightCardX + 42, cardY + 22);

      pdf.setTextColor(100, 116, 139); pdf.setFont('helvetica', 'normal'); pdf.text(translate('Transport:'), rightCardX + 5, cardY + 27);
      pdf.setTextColor(15, 23, 42); pdf.setFont('helvetica', 'bold'); pdf.text(translate('AC Luxury Coach'), rightCardX + 42, cardY + 27);

      pdf.setTextColor(100, 116, 139); pdf.setFont('helvetica', 'normal'); pdf.text(translate('Service Guarantee:'), rightCardX + 5, cardY + 32);
      pdf.setTextColor(5, 150, 105); pdf.setFont('helvetica', 'bold'); pdf.text(translate('5-Star Premier Rated'), rightCardX + 42, cardY + 32);
    }
    ...
    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(7.5);
    pdf.text(translate('ARRANGEMENT HIGHLIGHTS & ITINERARY NOTE'), margin + 5, noteY + 5);

    pdf.setTextColor(71, 85, 105);
    pdf.setFont('helvetica', 'normal');
    pdf.setFontSize(7.5);
    const rawNote = bookingData.description || `Confirmed reservation voucher for ${bookingData.customerName}. Includes all scheduled luxury transportation, dedicated host arrangements, and priority access passes.`;
    const noteText = translateDescription(rawNote);
    ...
    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(7.5);
    pdf.text(translate('DESTINATION MAP & ITINERARY LOCATION PASS'), margin + 5, mapY + 5);

    pdf.setTextColor(0, 145, 234);
    pdf.setFontSize(7);
    pdf.text(translate('GPS VERIFIED LOCATION'), pageWidth - margin - 5, mapY + 5, { align: 'right' });
    ...
    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(6);
    pdf.text(translate('DESTINATION PIN'), mapBoxX + 26, mapBoxY + 12, { align: 'center' });
    ...
    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(9);
    pdf.text(translate(destinationLoc).substring(0, 50), locTextX, mapY + 14);

    pdf.setTextColor(100, 116, 139);
    pdf.setFont('helvetica', 'normal');
    pdf.setFontSize(7.5);
    pdf.text(translate('Primary Arrival & Check-In Point • Google Maps Static API Pass'), locTextX, mapY + 19);

    pdf.setTextColor(5, 150, 105);
    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(7);
    pdf.text(`${translate('Coordinates:')} ${getCoordinates(destinationLoc)} • ${translate('Verified Destination')}`, locTextX, mapY + 23);
    ...
    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(7.5);
    pdf.text(translate('Payment Summary & Fare Breakdown'), margin + 5, tableY + 5.5);

    pdf.setTextColor(0, 145, 234);
    pdf.text(translate('Paid via Stripe / Card'), pageWidth - margin - 5, tableY + 5.5, { align: 'right' });

    pdf.setTextColor(71, 85, 105);
    pdf.setFont('helvetica', 'normal');
    pdf.setFontSize(8);
    pdf.text(`${translate('Base Rate / Fare')} (${bookingData.guestsCount || 1} ${translate('Traveler / Unit')})`, margin + 5, currentY);
    pdf.setTextColor(15, 23, 42);
    pdf.setFont('helvetica', 'bold');
    ...
    if (hotelAmount) {
      pdf.setTextColor(71, 85, 105);
      pdf.setFont('helvetica', 'normal');
      pdf.text(translate('Hotel Accommodation Add-on'), margin + 5, currentY);
      pdf.setTextColor(15, 23, 42);
      pdf.setFont('helvetica', 'bold');
    ...
    if (carAmount) {
      pdf.setTextColor(71, 85, 105);
      pdf.setFont('helvetica', 'normal');
      pdf.text(translate('Private Vehicle Rental Add-on'), margin + 5, currentY);
      pdf.setTextColor(15, 23, 42);
      pdf.setFont('helvetica', 'bold');
    ...
    pdf.setTextColor(71, 85, 105);
    pdf.setFont('helvetica', 'normal');
    pdf.text(translate('Taxes, Tourism Levies & Service Fees'), margin + 5, currentY);
    pdf.setTextColor(15, 23, 42);
    pdf.setFont('helvetica', 'bold');
    ...
    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(9.5);
    pdf.text(translate('Total Amount Charged'), margin + 5, currentY);
    ...
    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(7.5);
    pdf.text(translate('Important Traveler Instructions:'), margin, footerY + 5);

    pdf.setTextColor(100, 116, 139);
    pdf.setFont('helvetica', 'normal');
    pdf.setFontSize(7);
    pdf.text(`• ${translate('Please present this physical PDF voucher or digital screen pass upon arrival.')}`, margin, footerY + 10);
    pdf.text(`• ${translate('For 24/7 concierge assistance: support@premiertourbooking.com | +1 800-555-PREMIER.')}`, margin, footerY + 15);
    pdf.text(`• ${translate('Non-transferable travel pass. Photo ID matching lead guest required at check-in.')}`, margin, footerY + 20);
    ...
    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(6.5);
    pdf.text(`${translate('VERIFIED')} • ${bookingData.bookingRef}`, bcX + 28, bcY + 19, { align: 'center' });
    ...
    pdf.setFont('helvetica', 'normal');
    pdf.setFontSize(7);
    pdf.text(translate('Official Verified Travel Pass • Premier Tour Booking Ltd. • ABTA & ATOL Bonded • All Rights Reserved.'), pageWidth / 2, 274, { align: 'center' });
    ...
    <AnimatePresence>
      {isOpen && booking && (
        <motion.div 
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 z-[100000] flex items-center justify-center p-2 sm:p-4 md:p-6 bg-slate-950/95 backdrop-blur-2xl overflow-y-auto w-screen h-screen start-0 top-0"
        >
          ...
          <div className="flex items-center justify-between px-6 py-4 bg-slate-950 text-white border-b border-sky-900/50 shrink-0">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <span>{copiedLink ? translate('Copied!') : shareSuccess ? translate('Shared!') : translate('Share')}</span>
              </button>
              ...
              <button
                onClick={onClose}
                className="p-1.5 bg-slate-800 hover:bg-sky-900/60 text-slate-400 hover:text-white rounded-xl transition-colors cursor-pointer ms-1"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
        ...
        <div className="overflow-y-auto p-4 sm:p-8 bg-slate-100 dark:bg-slate-950 flex-1 custom-scrollbar">
          ...
          <div 
            ref={voucherRef}
            className="w-full max-w-2xl mx-auto bg-white text-slate-900 p-8 sm:p-10 rounded-2xl shadow-xl border border-slate-200 space-y-6 print:shadow-none print:border-0 print:p-0 notranslate"
            translate="no"
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-6 border-b-2 border-slate-900">
              <div className="space-y-1">
                ...
                <p className="text-2xs text-slate-500 font-semibold uppercase tracking-widest ps-10">
              ...
              <div className="text-start sm:text-end space-y-1 bg-slate-50 sm:bg-transparent p-3 sm:p-0 rounded-xl w-full sm:w-auto">
                <span className="inline-block px-3 py-1 bg-emerald-100 text-emerald-800 font-extrabold text-[10px] uppercase tracking-widest rounded-full border border-emerald-200">
                  ✓ {translate('Official Voucher & Receipt')}
                </span>
                <p className="text-xs font-mono font-black text-[#0091EA]">
                  {translate('REF')}: {booking.bookingRef}
                </p>
                <p className="text-2xs text-slate-400 font-medium">
                  {translate('Issued')}: {booking.createdAt ? new Date(booking.createdAt).toLocaleDateString() : new Date().toLocaleDateString()}
                </p>
              </div>
            </div>
            ...
            <div className="bg-slate-900 text-white rounded-2xl p-6 space-y-4 shadow-md relative overflow-hidden">
              ...
              <div className="space-y-1">
                ...
                {booking.subtitle && (
                  <p className="text-xs text-slate-300 font-medium">{translate(booking.subtitle)}</p>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-3 border-t border-slate-800 text-xs">
                ...
                <div>
                  ...
                  <span className="font-extrabold text-white mt-0.5 block">{translate(booking.startDate)}</span>
                </div>

                <div>
                  <span className="text-[10px] uppercase text-slate-400 font-bold block">{translate(`Start Time`)}</span>
                  <span className="font-extrabold text-white mt-0.5 block">{translate(resolvedStartTime)}</span>
                </div>

                <div>
                  <span className="text-[10px] uppercase text-slate-400 font-bold block">{translate(`Duration`)}</span>
                  <span className="font-extrabold text-sky-300 mt-0.5 block">{translate(resolvedDuration)}</span>
                </div>
              </div>
            </div>
              ...
              <div className="p-4 rounded-xl border border-slate-200 bg-slate-50/70 space-y-2">
                ...
                <div className="space-y-1 text-xs">
                  ...
                  <p className="text-slate-500 font-semibold pt-1">
                    {translate('Party Size')}: <span className="font-extrabold text-slate-800">{booking.guestsCount || 1} {translate('Person(s)')}</span>
                  </p>
                </div>
              </div>
                  ...
                  {booking.type === 'hotel' ? (
                    <>
                      <div className="flex justify-between items-center">
                        <span className="text-slate-500 font-medium">{translate('Assigned Room:')}</span>
                        <span className="font-black text-[#0091EA] bg-sky-50 px-2 py-0.5 rounded border border-sky-200">
                          {translate(resolvedRoomNumber)}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-slate-500 font-medium">{translate('Room Category:')}</span>
                        <span className="font-bold text-slate-800">{translate(resolvedRoomType)}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-slate-500 font-medium">{translate('Check-Out Date:')}</span>
                        <span className="font-bold text-slate-800">{translate(booking.endDate || 'Standard +3 Days')}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-slate-500 font-medium">{translate('Inclusions:')}</span>
                    ...
                    <>
                      <div className="flex justify-between items-center">
                        <span className="text-slate-500 font-medium">{translate('Vehicle Category:')}</span>
                        <span className="font-bold text-slate-800">{translate(booking.carCategory || 'Luxury Vehicle')}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-slate-500 font-medium">{translate('Pick-Up Location:')}</span>
                        <span className="font-bold text-[#0091EA]">{translate(booking.pickupLocation || 'Colombo International Airport')}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-slate-500 font-medium">{translate('Drop-Off Location:')}</span>
                        <span className="font-bold text-emerald-600">{translate(booking.dropoffLocation || booking.pickupLocation || 'Destination Hotel')}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-slate-500 font-medium">{translate('Return Date:')}</span>
                        <span className="font-bold text-slate-800">{translate(booking.endDate || booking.startDate)}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-slate-500 font-medium">{translate('Transmission:')}</span>
                        <span className="font-bold text-slate-800">{translate(booking.carTransmission || booking.transmission || 'Automatic')}</span>
                      </div>
                    </>
                  ) : booking.type === 'flight' ? (
                    <>
                      <div className="flex justify-between items-center">
                        <span className="text-slate-500 font-medium">{translate('Airline:')}</span>
                        <span className="font-bold text-slate-800">{translate(booking.flightAirline || 'Emirates Air')}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-slate-500 font-medium">{translate('Route:')}</span>
                        <span className="font-bold text-slate-800">{translate(booking.flightFrom || 'CMB')} ➔ {translate(booking.flightTo || 'DXB')}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-slate-500 font-medium">{translate('Cabin Class:')}</span>
                    ...
                    <>
                      <div className="flex justify-between items-center">
                        <span className="text-slate-500 font-medium">{translate('Package Type:')}</span>
                        <span className="font-bold text-[#0091EA]">{translate(booking.category || 'All-Inclusive Tour')}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-slate-500 font-medium">{translate('Scheduled Guide:')}</span>
                        <span className="font-bold text-slate-800">{translate(`Licensed English Guide`)}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-slate-500 font-medium">{translate('Transport:')}</span>
            ...
            {booking.description && (
              <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 text-xs text-slate-600 space-y-1">
                ...
                <p className="leading-relaxed font-medium">{translateDescription(booking.description)}</p>
              </div>
            )}
            ...
            <div className="p-4 rounded-xl border border-slate-200 bg-slate-50/80 space-y-3">
              ...
              <div className="relative h-44 rounded-lg overflow-hidden border border-slate-300 bg-slate-900 group">
                ...
                <div className="absolute bottom-2 start-2 end-2 pointer-events-none flex items-center justify-between">
            ...
            <div className="border border-slate-200 rounded-xl overflow-hidden text-xs">
              <div className="bg-slate-100 p-3 font-extrabold text-slate-800 border-b border-slate-200 flex justify-between">
                ...
                <span className="text-[#0091EA]">{translate('Paid via Stripe / Card')}</span>
              </div>

              <div className="p-4 space-y-2.5">
                <div className="flex justify-between text-slate-600 font-medium">
                  <span>{translate('Base Rate / Fare')} ({booking.guestsCount || 1} {translate('Traveler / Unit')})</span>
            ...
            <div className="pt-4 border-t border-slate-200 flex flex-col sm:flex-row justify-between items-center gap-4 text-2xs text-slate-400">
              ...
              <div className="space-y-1 text-center sm:text-start">
                <p className="font-bold text-slate-600">{translate('Important Traveler Instructions:')}</p>
                <p>• {translate('Please present this physical PDF voucher or digital screen pass upon arrival.')}</p>
                <p>• {translate('For 24/7 concierge assistance, contact support@premiertourbooking.com or +1 800-555-PREMIER.')}</p>
              </div>
            ...
            <div className="p-6 overflow-y-auto space-y-4 text-xs">
              ...
              <div className="flex items-start gap-3 p-3.5 bg-slate-50 dark:bg-slate-800/60 rounded-2xl border border-slate-200 dark:border-slate-700">
                ...
                <div>
                  ...
                  <p className="text-slate-500 dark:text-slate-400 text-[11px] mt-0.5">
                    {translate(`Click the`)} <strong className="text-purple-600 dark:text-purple-400">{translate(`Download PDF`)}</strong> {translate('button on this voucher screen. Save the file (e.g.,')} <code className="bg-slate-200 dark:bg-slate-950 px-1 py-0.5 rounded font-mono text-[10px]">{booking?.bookingRef || 'PB-VOUCHER'}.pdf</code>) {translate('to your desktop or downloads folder.')}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 p-3.5 bg-slate-50 dark:bg-slate-800/60 rounded-2xl border border-slate-200 dark:border-slate-700">
                ...
                <div>
                  ...
                  <p className="text-slate-500 dark:text-slate-400 text-[11px] mt-0.5">
                    {translate(`Open`)} <a href="https://hpanel.hostinger.com" target="_blank" rel="noopener noreferrer" className="text-purple-600 dark:text-purple-400 font-bold underline inline-flex items-center gap-0.5">hpanel.hostinger.com <ExternalLink className="w-3 h-3 inline" /></a> {translate('and sign in to your Hostinger web hosting account.')}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 p-3.5 bg-slate-50 dark:bg-slate-800/60 rounded-2xl border border-slate-200 dark:border-slate-700">
                ...
                <div>
                  ...
                  <p className="text-slate-500 dark:text-slate-400 text-[11px] mt-0.5">
                    {translate(`Go to`)} <strong className="text-slate-800 dark:text-slate-200">{translate(`Websites`)}</strong> {translate('&rarr; Click')} <strong className="text-slate-800 dark:text-slate-200">{translate(`Dashboard`)}</strong> {translate('next to your domain name &rarr; Click')} <strong className="text-purple-600 dark:text-purple-400">{translate(`File Manager`)}</strong> {translate('under Files.')}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 p-3.5 bg-slate-50 dark:bg-slate-800/60 rounded-2xl border border-slate-200 dark:border-slate-700">
                ...
                <div>
                  <h4 className="font-extrabold text-slate-900 dark:text-white text-xs">{translate('Navigate to public_html & Upload PDF')}</h4>
                  <p className="text-slate-500 dark:text-slate-400 text-[11px] mt-0.5">
                    {translate(`Open`)} <code className="bg-slate-200 dark:bg-slate-950 px-1 py-0.5 rounded font-mono text-[10px]">public_html</code> {translate('(or create a subfolder named')} <code className="bg-slate-200 dark:bg-slate-950 px-1 py-0.5 rounded font-mono text-[10px]">vouchers</code>{translate('). Click the')} <strong className="text-emerald-600 dark:text-emerald-400">{translate(`Upload File`)}</strong> {translate('icon at top right and select your downloaded PDF.')}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 p-3.5 bg-emerald-50 dark:bg-emerald-950/40 rounded-2xl border border-emerald-200 dark:border-emerald-800">
                ...
                <div>
                  ...
                  <p className="text-emerald-700 dark:text-emerald-400 text-[11px] mt-0.5">
                    {translate('Your PDF booking pass is now hosted live on your website! You or your travelers can access it at:')}
                  </p>
